    ...
    >
      <template #content="{ currentIndex: activeIndex, handleItemClick, pauseAutoRotation, resumeAutoRotation }">
        <div class="header-section">
          <div class="title">
            {{ t('ProductionFeature.title') }}
          </div>
          <div class="title">
            {{ t('ProductionFeature.description') }}
          </div>
        </div>
        <div :class="[
          ...
          <div class="text-wrap">
            ...
            <div class="actions-section">
              <StartReadingButton :buttonText="t('ProductionFeature.buttons.startReading')" />
            </div>
          </div>
...
// 模块类型
const moduleType = ref('ProductionFeature');

// 特性项目，从当前语言的翻译文件中获取
const featureItems = computed(() => {
  const features = ['cloudSync', 'folders', 'sorting', 'citations'];
  return features.map(key => {
    const imgUrl = getImageUrl(t(`ProductionFeature.features.${key}.imgUrl`));
    
    return {
      i18nKey: key,
      title: t(`ProductionFeature.features.${key}.title`),
      description: t(`ProductionFeature.features.${key}.description`),
      imgUrl,
      iconUrl: getImageUrl(t(`ProductionFeature.features.${key}.iconUrl`))
    };
  });
});
