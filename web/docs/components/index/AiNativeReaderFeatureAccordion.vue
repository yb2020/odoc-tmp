    ...
    >
      <template #content="{ currentIndex: activeIndex, handleItemClick, pauseAutoRotation, resumeAutoRotation }">
        <div class="header-section">
          <div class="title">
            {{ t('AiNativeReaderFeature.title') }}
          </div>
          <div class="title">
            {{ t('AiNativeReaderFeature.description') }}
          </div>
        </div>
        <div :class="[
          ...
          <div class="text-wrap">
            ...
            <div class="actions-section">
              <StartReadingButton :buttonText="t('AiNativeReaderFeature.buttons.startReading')" />
            </div>
          </div>
...
// 模块类型
const moduleType = ref('AiNativeReaderFeature');

// 特性项目，从当前语言的翻译文件中获取
const featureItems = computed(() => {
  const features = ['sideBySide', 'jumpToReferences', 'noteTakingFormats'];
  return features.map(key => {
    const imgUrl = getImageUrl(t(`AiNativeReaderFeature.features.${key}.imgUrl`));
    
    return {
      i18nKey: key,
      title: t(`AiNativeReaderFeature.features.${key}.title`),
      description: t(`AiNativeReaderFeature.features.${key}.description`),
      imgUrl,
      iconUrl: getImageUrl(t(`AiNativeReaderFeature.features.${key}.iconUrl`))
    };
  });
});
