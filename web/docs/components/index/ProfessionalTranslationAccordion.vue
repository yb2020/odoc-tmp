    ...
    >
      <template #content="{ currentIndex: activeIndex, handleItemClick, pauseAutoRotation, resumeAutoRotation }">
        <div class="header-section">
          <div class="title">
            {{ t('ProfessionalTranslationFeature.title') }}
          </div>
          <div class="description">
            {{ t('ProfessionalTranslationFeature.description') }}
          </div>
        </div>
        <div :class="[
          ...
          <div class="text-wrap">
            ...
            <div class="actions-section">
              <StartReadingButton :buttonText="t('ProfessionalTranslationFeature.buttons.startReading')" />
            </div>
          </div>
...
// 模块类型
const moduleType = ref('ProfessionalTranslationFeature');

// 特性项目，从当前语言的翻译文件中获取
const featureItems = computed(() => {
  const features = ['fullText', 'highlight', 'terminology'];
  return features.map(key => ({
    title: t(`ProfessionalTranslationFeature.features.${key}.title`),
    description: t(`ProfessionalTranslationFeature.features.${key}.description`),
    imgUrl: getImageUrl(t(`ProfessionalTranslationFeature.features.${key}.imgUrl`)),
    iconUrl: getImageUrl(t(`ProfessionalTranslationFeature.features.${key}.iconUrl`))
  }));
});
