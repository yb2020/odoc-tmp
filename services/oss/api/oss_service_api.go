...
// OssServiceAPI OSS API处理器
type OssServiceAPI struct {
...
// NewOSSServiceAPI 创建OSS API处理器
func NewOSSServiceAPI(logger logging.Logger, tracer opentracing.Tracer, config *config.Config, ossService service.OssServiceInterface) *OssServiceAPI {
	return &OssServiceAPI{
		logger:     logger,
		tracer:     tracer,
		config:     config,
...
// GetS3UploadToken 处理文件上传请求
func (api *OssServiceAPI) GetS3UploadToken(c *gin.Context) {
...
// 获取解析后的文件的临时下载url
func (api *OssServiceAPI) GetParseDownloadTempUrl(c *gin.Context) {
...
// 获取pdf文件的临时下载url
func (api *OssServiceAPI) GetPdfDownloadTempUrl(c *gin.Context) {
	span, ctx := opentracing.StartSpanFromContextWithTracer(c.Request.Context(), api.tracer, "OssServiceAPI.GetDownloadTeamUrl")
	defer span.Finish()
	...
	ossRecord, err := api.ossService.GetSuccessRecordByFileSHA256(ctx, req.GetFileSHA256())
	if err != nil {
		response.ErrorNoData(c, err.Error())
		return
	}
	if ossRecord == nil {
		response.ErrorNoData(c, "oss record is nil")
		return
	}
	url, err := api.ossService.GetFileTemporaryURL(ctx, ossConstant.BucketTypeToEnum(api.config, ossRecord.BucketName), ossRecord.ObjectKey, 600)
	if err != nil {
		response.ErrorNoData(c, err.Error())
		return
	}
...
// 测试文件上传到S3
func (api *OssServiceAPI) UploadToS3(c *gin.Context) {
	span, ctx := opentracing.StartSpanFromContextWithTracer(c.Request.Context(), api.tracer, "OssServiceAPI.UploadToS3")
	defer span.Finish()
